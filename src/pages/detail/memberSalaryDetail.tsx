...
export default function MemberSalaryDetail() {
    ...
    const initialMonth = useMemo(() => {
        if (year && month) return dayjs(`${year}-${month}-01`);
        return dayjs();
    }, [year, month]);
    ...
    const stats = useMemo(() => {
        ...
        schedules.forEach(sch => {
            ...
            totalHours += dur;
            const base = sch.hourlyWage ?? member?.hourlyWage ?? 10000;
            ...
            if (member?.weeklyAllowance) {
                breakdown['주휴 수당'].amount += base * 1.1;
                breakdown['주휴 수당'].hours += 1;
                breakdown['주휴 수당'].rate = base;
            }
            if (member?.nightAllowance && (start.hour() < 6 || end.hour() > 22)) {
                const extra = base * dur * ((member.nightRate ?? 150) - 100) / 100;
                breakdown['야간 수당'].amount += extra;
                breakdown['야간 수당'].hours += dur;
                breakdown['야간 수당'].rate = base;
            }
            if (member?.overtimeAllowance && dur > 8) {
                const extra = base * (dur - 8) * ((member.overtimeRate ?? 150) - 100) / 100;
                breakdown['초과근무 수당'].amount += extra;
                breakdown['초과근무 수당'].hours += dur - 8;
                breakdown['초과근무 수당'].rate = base;
            }
            if (member?.holidayAllowance && start.day() === 0) {
                const extra = base * dur * ((member.holidayRate ?? 150) - 100) / 100;
        ...
        <ScrollView style={styles.container}>
            ...
            <View style={styles.outerBox}>
                ...
                <Text style={styles.workplaceName}>
                    {member?.name + currentMonth.format(' M월') + " 급여 상세"}
                </Text>
