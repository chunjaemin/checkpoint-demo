...
import { View, Text, TouchableOpacity, Pressable, StyleSheet, InteractionManager, Alert } from 'react-native';
import { useUserStore } from '@/scripts/store/userStore';
import { useCalTypeStore, useEditDateStore } from '@/features/calendar/team';
...
export default function WeekEdit() {
    ...
    const [selectedCellsMap, setSelectedCellsMap] = useState<Record<string, Record<string, number[]>>>({});
    ...
    function isAllCellsEmpty(selectedCellsMap: Record<string, Record<string, number[]>>) {
    ...
    const handleSave = () => {
        ...
        for (const col in cells) {
            ...
            if (!cells[col] || cells[col].length === 0) continue;

            const rows = [...cells[col]].sort((a, b) => a - b);
            let startIdx = null;
        ...
        <View style={{ flex: 1 }}>
            ...
            >
                <View style={{ backgroundColor: 'white', padding: 20, paddingBottom: 100, borderTopLeftRadius: 12, borderTopRightRadius: 12 }}>
                    ...
                    {Array.isArray(spaces) && spaces?.map((space: any, i: number) => (
                        <View key={i} style={{ flexDirection: 'row', alignItems: 'center', overflow: 'hidden', marginBottom: 8 }}>
                            <View style={{
                                width: 6,
                                height: '100%',
                                backgroundColor: scheduleColors[(space.color as keyof typeof scheduleColors)]?.main ?? '#ccc',
